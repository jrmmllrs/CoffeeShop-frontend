    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
        <div className="text-center">
          ...
          <p className="text-xl text-amber-800 font-medium">Brewing your reports...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 lg:gap-6">
            <div>
              ...
              <p className="text-amber-700">Track your coffee shop's performance</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              <div className="bg-gradient-to-br from-emerald-500 to-green-600 rounded-2xl shadow-xl p-6 text-white transform hover:scale-105 transition-transform">
                ...
                <p className="text-2xl sm:text-3xl font-bold mb-2">{formatCurrency(stats.totalRevenue)}</p>
                <p className="text-xs text-emerald-100">Today: {formatCurrency(stats.todayRevenue)}</p>
              </div>
              ...
              <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-xl p-6 text-white transform hover:scale-105 transition-transform">
                ...
                <p className="text-2xl sm:text-3xl font-bold mb-2">{stats.totalSales}</p>
                <p className="text-xs text-blue-100">Today: {stats.todaySales} orders</p>
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl shadow-xl p-6 text-white transform hover:scale-105 transition-transform">
                ...
                <p className="text-2xl sm:text-3xl font-bold mb-2">{stats.totalProducts}</p>
                <p className="text-xs text-purple-100">{stats.lowStockProducts} low stock</p>
              </div>
              ...
              <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl shadow-xl p-6 text-white transform hover:scale-105 transition-transform">
                <div className="text-3xl mb-2">⚡</div>
                <p className="text-amber-100 text-sm font-medium mb-1">Avg. Sale</p>
                <p className="text-2xl sm:text-3xl font-bold mb-2">{formatCurrency(stats.averageSale)}</p>
              ...
              <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
                ...
                <div className="space-y-3">
                  {reportData.sales.slice(0, 7).map((day, index) => (
                    <div key={index} className="bg-amber-50 rounded-xl p-4 hover:bg-amber-100 transition-colors">
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-semibold text-amber-900">{formatDate(day.date)}</span>
                        <div className="text-right">
                          <p className="text-lg font-bold text-emerald-600">{formatCurrency(day.total_revenue)}</p>
                          <p className="text-xs text-amber-700">{day.total_sales} orders</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
                ...
                <div className="space-y-3">
                  {lowStockAlerts.slice(0, 5).map((product, index) => (
                    <div key={index} className={`rounded-xl p-4 ${product.stock === 0 ? 'bg-red-50 border-2 border-red-200' : 'bg-amber-50 border-2 border-amber-200'}`}>
                      <div className="flex justify-between items-center">
                        <div className="flex-1">
                  ...
                  {lowStockAlerts.length === 0 && (
                    <div className="text-center py-8 text-emerald-600">
                      <div className="text-4xl mb-2">✅</div>
                      <p className="font-semibold">All products well stocked!</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
                ...
                <div className="space-y-3">
                  {topProducts.map((product, index) => (
                    <div key={index} className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-4 border-2 border-amber-100">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-amber-600 capitalize">{product.category}</p>
                          </div>
                        </div>
                        <div className="text-right ml-4">
                          <p className="text-lg font-bold text-emerald-600">{product.sales_count || 0}</p>
                      ...
                      <div key={index} className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-4 border-2 border-amber-100">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold text-amber-900">{methodInfo.name}</span>
                          </div>
                          <div className="text-right">
                            <p className="text-lg font-bold text-emerald-600">{formatCurrency(method.total_amount)}</p>
                            <p className="text-xs text-amber-700">{method.transaction_count} txns</p>
                          </div>
                        </div>
                        <div className="w-full bg-amber-200 rounded-full h-2 overflow-hidden">
                          <div className="bg-gradient-to-r from-amber-500 to-orange-500 h-2 rounded-full transition-all duration-500" style={{ width: `${method.percentage || 0}%` }}></div>
                        </div>
                        <p className="text-xs text-amber-600 mt-1 text-right">{method.percentage || 0}%</p>
                      </div>
                    );
                  })}
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                  {reportData.sales.map((day, index) => (
                    <div key={index} className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-4 border-2 border-amber-100">
                      <div className="flex justify-between items-start mb-3">
                        <span className="font-bold text-amber-900">{formatDate(day.date)}</span>
                        <span className="text-xl font-bold text-emerald-600">{formatCurrency(day.total_revenue)}</span>
                      </div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="bg-white rounded-lg p-2">
                          <p className="text-amber-600 text-xs">Total Sales</p>
                          <p className="font-bold text-amber-900">{day.total_sales}</p>
                        </div>
                        <div className="bg-white rounded-lg p-2">
                          <p className="text-amber-600 text-xs">Avg Sale</p>
                          <p className="font-bold text-amber-900">{formatCurrency(day.average_sale)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                  {reportData.sales.length === 0 && (
                    <div className="text-center py-12 text-amber-600">
                      ...
                      <p className="font-semibold">No sales data for selected period</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
                ...
                <div className="space-y-4">
                  {reportData.paymentAnalytics.map((method, index) => {
                    const methodInfo = getPaymentMethodInfo(method.payment_method);
                    return (
                      <div key={index} className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-5 border-2 border-amber-100">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <span className="text-3xl">{methodInfo.icon}</span>
                            <div>
                              <p className="font-bold text-amber-900 text-lg">{methodInfo.name}</p>
                              <p className="text-sm text-amber-600">{method.transaction_count} transactions</p>
                            </div>
                          </div>
                          <span className="text-xl font-bold text-emerald-600">{formatCurrency(method.total_amount)}</span>
                        </div>
                        <div className="w-full bg-amber-200 rounded-full h-3 overflow-hidden">
                          <div className="bg-gradient-to-r from-emerald-500 to-green-600 h-3 rounded-full transition-all duration-500" style={{ width: `${method.percentage || 0}%` }}></div>
                        </div>
                        <p className="text-sm text-amber-700 mt-2 text-right font-semibold">{method.percentage || 0}% of total revenue</p>
                      </div>
                    );
                  })}
                  {reportData.paymentAnalytics.length === 0 && (
              ...
              <div className="inline-block min-w-full align-middle">
                <table className="min-w-full divide-y-2 divide-amber-200">
                  <thead className="bg-gradient-to-r from-amber-100 to-orange-100">
                    <tr>
                      <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-amber-900 uppercase tracking-wider">Product</th>
                      <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-amber-900 uppercase tracking-wider">Category</th>
                      <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-amber-900 uppercase tracking-wider">Price</th>
                      <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-amber-900 uppercase tracking-wider">Stock</th>
                      <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-amber-900 uppercase tracking-wider">Sales</th>
                      <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-amber-900 uppercase tracking-wider">Status</th>
                    </tr>
                  </thead>
                    ...
                    {reportData.products.map((product, index) => (
                      <tr key={index} className="hover:bg-amber-50 transition-colors">
                        <td className="px-3 sm:px-4 py-4">
                          <div>
                            <p className="font-bold text-amber-900 text-sm sm:text-base">{product.name}</p>
                            {product.description && (
                              <p className="text-xs text-amber-600 truncate max-w-xs mt-1">{product.description}</p>
                            )}
                          </div>
                        </td>
                        <td className="px-3 sm:px-4 py-4">
                        ...
                        <td className="px-3 sm:px-4 py-4 font-bold text-emerald-600 text-sm sm:text-base">{formatCurrency(product.price)}</td>
                        <td className="px-3 sm:px-4 py-4">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl shadow-xl p-6 text-white text-center transform hover:scale-105 transition-transform">
                ...
                <p className="text-amber-100 text-sm font-medium mb-2">Low Stock Items</p>
              ...
              <div className="bg-gradient-to-br from-red-500 to-pink-600 rounded-2xl shadow-xl p-6 text-white text-center transform hover:scale-105 transition-transform">
                <div className="text-4xl mb-3">❌</div>
                <p className="text-red-100 text-sm font-medium mb-2">Out of Stock</p>
            ...
            <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {reportData.products.map((product, index) => (
                  <div key={index} className={`rounded-xl p-4 border-2 transform hover:scale-105 transition-all ${
                    product.stock === 0 ? "border-red-300 bg-gradient-to-br from-red-50 to-pink-50" :
                    product.stock <= 10 ? "border-amber-300 bg-gradient-to-br from-amber-50 to-orange-50" :
                    "border-emerald-300 bg-gradient-to-br from-emerald-50 to-green-50"
                  }`}>
                    <div className="flex justify-between items-start mb-3">
                      <h4 className="font-bold text-amber-900 flex-1 pr-2">{product.name}</h4>
                    ...
                    <p className="text-sm text-amber-600 capitalize mb-3 font-medium">{product.category}</p>
                    <div className="flex justify-between items-center">
                      <p className="text-lg font-bold text-emerald-600">{formatCurrency(product.price)}</p>
                      <p className="text-sm text-amber-700 font-semibold">{product.sales_count || 0} sold</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
                ...
                <div className="space-y-4">
                  {peakHours.map((hour, index) => (
                    <div key={index} className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-5 border-2 border-amber-200 transform hover:scale-105 transition-all">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <span className="text-3xl">{index === 0 ? "🥇" : index === 1 ? "🥈" : "🥉"}</span>
                          <div>
                            <p className="font-bold text-amber-900 text-lg">{formatHour(hour.hour)}</p>
                            <p className="text-sm text-amber-600">{hour.transaction_count} transactions</p>
                          </div>
                        </div>
                        <span className="text-xl font-bold text-emerald-600">{formatCurrency(hour.total_revenue)}</span>
                      </div>
                      <div className="w-full bg-amber-200 rounded-full h-3 overflow-hidden">
                        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 h-3 rounded-full transition-all duration-500"
                          style={{
                            width: `${(hour.total_revenue / Math.max(...reportData.hourlySales.map(h => h.total_revenue || 1))) * 100}%`
                          }}
                        ></div>
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                  {reportData.hourlySales.map((hour, index) => (
                    <div key={index} className="flex justify-between items-center p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl border border-amber-200 hover:border-amber-300 transition-all">
                      <div className="flex items-center gap-3">
                        <span className="text-2xl">⏰</span>
                        <span className="font-bold text-amber-900">{formatHour(hour.hour)}</span>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-emerald-600 text-lg">{formatCurrency(hour.total_revenue)}</p>
                        <p className="text-sm text-amber-700">{hour.transaction_count} orders</p>
                      </div>
                    </div>
                  ))}
                  {reportData.hourlySales.length === 0 && (
                    <div className="text-center py-12 text-amber-600">
                      <div className="text-5xl mb-3">⏰</div>
                      <p className="font-semibold">No hourly sales data available</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl border-2 border-amber-200 p-6">
              ...
              <div className="space-y-4">
                {reportData.categorySales.map((category, index) => (
                  <div key={index} className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-5 border-2 border-amber-200">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4">
                      <div>
                        <p className="font-bold text-amber-900 text-xl capitalize mb-1">{category.category || "Uncategorized"}</p>
                        <p className="text-sm text-amber-600">
                          {category.items_sold} items sold • {category.total_quantity} total quantity
                        </p>
                      </div>
                      <span className="text-2xl font-bold text-emerald-600">{formatCurrency(category.total_revenue)}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white rounded-lg p-3">
                        <p className="text-xs text-amber-600 mb-1">Average Price</p>
                        <p className="font-bold text-amber-900">{formatCurrency(category.total_revenue / (category.items_sold || 1))}</p>
                      </div>
                      <div className="bg-white rounded-lg p-3">
                        <p className="text-xs text-amber-600 mb-1">Items per Sale</p>
                        <p className="font-bold text-amber-900">{((category.total_quantity || 0) / (category.items_sold || 1)).toFixed(1)}</p>
                      </div>
                    </div>
                  </div>
                ))}
                {reportData.categorySales.length === 0 && (
                  <div className="text-center py-12 text-amber-600">
                    ...
                    <p className="font-semibold">No category sales data available</p>
                  </div>
                )}
              </div>
            </div>
                ...
                {reportData.categorySales.map((category, index) => {
                  ...
                  return (
                    <div key={index} className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-amber-900 capitalize text-lg">{category.category || "Uncategorized"}</span>
                        <span className="text-sm font-semibold text-amber-700">
                          {formatCurrency(category.total_revenue)} ({percentage.toFixed(1)}%)
                        </span>
                      </div>
